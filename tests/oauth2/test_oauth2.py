...
class OAuthSuite(BaseSuite):
    ...
    def create_oauth_provider(app):
        raise NotImplementedError("Each test class must" "implement this method.")
...
class TestRefreshToken(OAuthSuite):
    ...
    def test_refresh_token_in_password_grant(self):
        ...
        args = (data.get("scope").replace(" ", "+"), data.get("refresh_token"))
        url = "/oauth/token?grant_type=refresh_token" "&scope=%s&refresh_token=%s"
        url = url % args
        rv = self.client.get(
            url,
            headers={
...
class TestRevokeToken(OAuthSuite):
    ...
    def test_revoke_token(self):
        data = self.get_token()
        tok = Token.query.filter_by(refresh_token=data["refresh_token"]).first()
    ...
    def test_revoke_token_with_hint(self):
        data = self.get_token()
        tok = Token.query.filter_by(access_token=data["access_token"]).first()
        assert tok.access_token == data["access_token"]

        revoke_url = "/oauth/revoke"
        args = {"token": data["access_token"], "token_type_hint": "access_token"}
        self.client.post(
            revoke_url,
            data=args,
            headers={
...
class TestCredentialAuth(OAuthSuite):
    ...
    def test_get_access_token(self):
        url = "/oauth/token?grant_type=client_credentials" "&scope=email+address"
        rv = self.client.get(
            url,
            headers={
                "Authorization": "Basic %s" % auth_code,
    ...
    def test_invalid_auth_header(self):
        url = "/oauth/token?grant_type=client_credentials" "&scope=email+address"
        rv = self.client.get(url, headers={"Authorization": "Basic foobar"})
...
class TestConfidentialClient(OAuthSuite):
    ...
    def test_get_access_token(self):
        url = "/oauth/token?grant_type=authorization_code&code=12345" "&scope=email"
        rv = self.client.get(url, headers={"Authorization": "Basic %s" % auth_code})
        assert b"access_token" in rv.data

    def test_invalid_grant(self):
        url = "/oauth/token?grant_type=authorization_code&code=54321" "&scope=email"
        rv = self.client.get(url, headers={"Authorization": "Basic %s" % auth_code})
        assert b"invalid_grant" in rv.data

    def test_invalid_client(self):
        url = "/oauth/token?grant_type=authorization_code&code=12345" "&scope=email"
        rv = self.client.get(url, headers={"Authorization": "Basic %s" % ("foo")})
        assert b"invalid_client" in rv.data
