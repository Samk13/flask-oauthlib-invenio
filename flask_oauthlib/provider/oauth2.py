            ...
            if request.method in ("GET", "HEAD"):
                ...
                try:
                    ...
                    return self._on_exception(
                        e, add_params_to_uri(self.error_uri, {"error": str(e)})
                    )
...
class OAuth2RequestValidator(RequestValidator):
    ...
    def validate_bearer_token(self, token, scopes, request):
        ...
        # validate expires
        if (
            tok.expires is not None
            and datetime.datetime.now(datetime.timezone.utc) > tok.expires
        ):
    ...
    def validate_code(self, client_id, code, client, request, *args, **kwargs):
        ...
        if not grant:
            ...
            return False
        if (
            hasattr(grant, "expires")
            and datetime.datetime.now(datetime.timezone.utc) > grant.expires
        ):
